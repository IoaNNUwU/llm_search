...
try {
    ...
    if (!cancel_project_evaluation($pdo, $projectId)) {
        json_response(['error' => 'No active evaluation to cancel'], 409);
    }

    json_response(['ok' => true, 'project_id' => $projectId, 'status' => 'cancelled']);
} catch (Throwable $e) {
